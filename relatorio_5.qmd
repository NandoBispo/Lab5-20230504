...
for (i in 1:12){
  if (fit_anova3_cna[i,6]=="0,0000") {
    fit_anova3_cna[i,6] = "<0,0001"
  }  
}
...
Comummente nas avaliações na área de saúde o peso isoladamente é menos significativo que quando dividido pelo quadrado da altura, constituindo o que se convencionou de Índice de Massa Corpórea (IMC). Refazendo os calculos anteriores para essa nova variável temos o que se pode ver nas tabelas a seguir.
...
# Com remoção de NAs
dados4$IMC <- dados4$weight/(dados4$height^2)

mFit4.0sna <- lm(IMC ~ chol + stab_glu + hdl + ratio + glyhb + age + bp_1s + bp_1d + waist + hip, data = dados4)

mFit4.1sna <- lm(IMC ~  chol + stab_glu + hdl + age +  bp_1s + bp_1d + waist + hip, data = dados4)
...
A análise da Tabela 9 indica que o peso precisa ser avaliado em função da altura para desterminar a razão colesterol total e colesterol bom, uma vez que na ponderação do IMC esta condição deixa de estar significativamente correlacionada. Já para a Hemoglobina glicada o peso do paciente ou seu IMC estão relacionados aos resultados obtidos de forma significativa. Para todos os demais a correlação não é evidente.
