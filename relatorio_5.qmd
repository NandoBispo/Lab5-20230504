...
Para facilitar a compreenção das medidas apresentadas na Tabela 8, a Figura 8 mostra graficamente estas distribuições.
...
(g1 + g2 + g3 + g4 + g5 + g7 + g9 + g10 + g11 + g12 + g13 + g14) +
  plot_layout(nrow = 3) + 
  plot_annotation(
    title = "Figura 8: Histogramas das variáveis em análise.") &
    # tag_levels = c("A", "1"), tag_prefix = "Sub Fig. ", tag_sep = ".",
    # tag_suffix = ":") 
   theme(
     plot.tag.position = c(0, 1),
...
Pode-se realizar a análise de disperção dos dados por meio de gráficos do tipo BoxPlot, como se vê na Figura 9.
...
b1 + b2 + b3 + b4 + b5 + b6 + b7 + b8 + b9 + b10 + b11 + b12 +  
  plot_layout(ncol = 3) + 
  plot_annotation(
    title = "Figura 9: BoxPlot das variáveis em análise.") &
    # # theme = theme_minimal(plot.title = element_text(size = 10)),
    # tag_levels = c("A", "1"), tag_prefix = "Sub Fig. ", tag_sep = ".",
    # tag_suffix = ":") &
  theme(
...
Pode-se verificar pela Figura 9 que há diversos valores atipicos (*outlayers*), entretanto sem conhecimento especializado da fisiologia é temerário prescindir destas observações. Assim, decidiu-se por não realizar qualquer tratamento para essa observação, abrindo entretanto a possibilidade futura após o parecer de um especialista.
...
A análise da Tabela 9 permite avaliar que apenas para a razão colesterol total e colesterol bom e para a Hemoglobina glicada o peso do paciente está relacionado aos resultados obtidos de forma significativa. Para todos os demais a correlação não é evidente.


#### Realizando a RLM para a variável IMC

Comummente nas avaliações na área de saúde o peso isoladamente é menos significativo que quando dividido pelo quadrado da altura, constituindo o que se convencionou de Índice de Massa Corpórea (IMC). Refazendo os calculos anteriores para essa nova variável temos o que se pode ver na tabela a seguir.
